  ...
  <div class="">
    <div class="items grid">
      <div class="grid-item" v-for="(item, i) in $store.state.items" :key="i">
        ...
        <h5 style="color: #27ae60;" >Rp. {{ $store.getters.setMoney(item.price) }}</h5>
    ...
    <div class="summary" v-if="Object.keys($store.getters.getSummary).length > 0">
      ...
      <div class="total" v-for="(item, key, index) in $store.state.summary" :key="index">
        ...
        <h4 class="font-weight-bold" style="color: #27ae60;">Rp. {{$store.getters.setMoney(item.sum)}}</h4>
      </div>
      <div class="line"></div>
      <div class="total" style="margin-bottom: 10px;">
        <h4 class="font-weight-bold" style="margin-top: 10px;">Total</h4>
        <h4 class="font-weight-bold" style="margin-top: 10px; color: #27ae60">Rp. {{ $store.getters.setMoney(Object.values($store.getters.getSummary).reduce((a,b) => a + b.sum, 0)) }}</h4>
      </div>
